...
export default function App(){
  ...
  const route=(x)=>{
    if(["home","explore","mall"].includes(x)) setScreen(x);
    else setScreen("explore");
  };
  return <SafeAreaView style={s.safe}><StatusBar barStyle="light-content" backgroundColor={C.bg}/>{screen==="home"?<HomeScreen go={route}/>:screen==="explore"?<ExploreScreen go={route}/>:<MallScreen go={route}/>}<BottomNav active={screen==="mall"?"explore":screen} onChange={setScreen}/></SafeAreaView>;
}
