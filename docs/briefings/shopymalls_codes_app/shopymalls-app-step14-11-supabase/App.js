...
export default function App(){
  ...
  const route=(x)=>{
    if(["home","explore","mall","store","deal","saved","profile","business","malldashboard","admin"].includes(x)) setScreen(x);
    else setScreen("explore");
  };
  return <SafeAreaView style={s.safe}><StatusBar barStyle="light-content" backgroundColor={C.bg}/>{screen==="home"?<HomeScreen go={route}/>:screen==="explore"?<ExploreScreen go={route}/>:screen==="mall"?<MallScreen go={route}/>:screen==="store"?<StoreScreen go={route}/>:screen==="deal"?<DealScreen go={route}/>:screen==="saved"?<SavedScreen go={route}/>:screen==="profile"?<ProfileScreen go={route}/>:screen==="business"?<RetailerDashboardScreen go={route}/>:screen==="malldashboard"?<MallDashboardScreen go={route}/>:screen==="admin"?<AdminDashboardScreen go={route}/>:<ExploreScreen go={route}/>}<BottomNav active={["mall","store","deal"].includes(screen)?"explore":screen==="saved"?"saved":screen==="profile"?"profile":screen==="malldashboard"?"explore":screen==="admin"?"explore":screen} onChange={setScreen}/></SafeAreaView>;
}
