...
export default function App(){
  ...
  const route=(x)=>{
    if(["home","explore","mall","store","deal"].includes(x)) setScreen(x);
    else setScreen("explore");
  };
  return <SafeAreaView style={s.safe}><StatusBar barStyle="light-content" backgroundColor={C.bg}/>{screen==="home"?<HomeScreen go={route}/>:screen==="explore"?<ExploreScreen go={route}/>:screen==="mall"?<MallScreen go={route}/>:screen==="store"?<StoreScreen go={route}/>:screen==="deal"?<DealScreen go={route}/>:<ExploreScreen go={route}/>}<BottomNav active={["mall","store","deal"].includes(screen)?"explore":screen} onChange={setScreen}/></SafeAreaView>;
}
